 ...
 * @hook save questions
 */
if (!function_exists('fw_ext_cannalisting_process_questions')) {

    function fw_ext_cannalisting_process_questions() {
		...
		if( !is_user_logged_in() ) {
			$json['type'] = 'error';
            $json['message'] = esc_html__('Please login before add your question.', 'cannalisting_core');
        ...
        $do_check = check_ajax_referer('cannalisting_question_answers_nounce', 'cannalisting_question_answers_nounce', false);
        if ($do_check == false) {
            $json['type'] = 'error';
		...
		if ( isset($_POST['category']) && empty( $_POST['category'] ) && $type === 'open' ) {
			$json['type'] = 'error';
            $json['message'] = esc_html__('Question category is required.', 'cannalisting_core');
            echo json_encode($json);
            die;
		}
        ...
        if (empty($_POST['question_title'])) {
            $json['type'] = 'error';
            $json['message'] = esc_html__('Question title field should not be empty.', 'cannalisting_core');
            echo json_encode($json);
            die;
        }

        $question_title = !empty($_POST['question_title']) ? esc_attr($_POST['question_title']) : esc_html__('unnamed', 'cannalisting_core');
        $question_detail = do_shortcode($_POST['question_description']);
        ...
        $json['type'] = 'success';
        $json['message'] = esc_html__('Question submit successfully.', 'cannalisting_core');
        echo json_encode($json);
        die;
    }

    add_action('wp_ajax_fw_ext_cannalisting_process_questions', 'fw_ext_cannalisting_process_questions');
    add_action('wp_ajax_nopriv_fw_ext_cannalisting_process_questions', 'fw_ext_cannalisting_process_questions');
}
 ...
 * @hook Save Answers
 */
if (!function_exists('fw_ext_cannalisting_process_answers')) {

    function fw_ext_cannalisting_process_answers() {
        global $current_user, $wp_roles, $userdata;
        $json = array();
        ...
        $do_check = check_ajax_referer('cannalisting_answers_nounce', 'cannalisting_answers_nounce', false);
        if ($do_check == false) {
            $json['type'] = 'error';
            $json['message'] = esc_html__('No kiddies please!', 'cannalisting_core');
            echo json_encode($json);
        ...
        if (empty($_POST['answer_description'])) {
            $json['type'] = 'error';
            $json['message'] = esc_html__('Answers description area should not be empty.', 'cannalisting_core');
            echo json_encode($json);
            die;
        }

        $answer_detail = do_shortcode($_POST['answer_description']);
        ...
        $json['type'] = 'success';
        $json['message'] = esc_html__('Answer submitted successfully.', 'cannalisting_core');
        echo json_encode($json);
        die;
    }

    add_action('wp_ajax_fw_ext_cannalisting_process_answers', 'fw_ext_cannalisting_process_answers');
    add_action('wp_ajax_nopriv_fw_ext_cannalisting_process_answers', 'fw_ext_cannalisting_process_answers');
}

/**
 * @hook update votes
 */
if (!function_exists('fw_ext_cannalisting_update_votes')) {

    function fw_ext_cannalisting_update_votes() {
        global $current_user, $wp_roles, $userdata;
        $json = array();
        ...
        $json['vote'] = $count;
        $json['type'] = 'success';
        $json['message'] = esc_html__('Your vote update.', 'cannalisting_core');
        echo json_encode($json);
        die;
    }

    add_action('wp_ajax_fw_ext_cannalisting_update_votes', 'fw_ext_cannalisting_update_votes');
    add_action('wp_ajax_nopriv_fw_ext_cannalisting_update_votes', 'fw_ext_cannalisting_update_votes');
}

/**
 * @hook update likes
 */
if (!function_exists('fw_ext_cannalisting_update_likes')) {

    function fw_ext_cannalisting_update_likes() {
        global $current_user, $wp_roles, $userdata;
        $json = array();
		$key	= !empty( $_POST['key'] ) ? esc_attr( $_POST['key'] ) : '';
		$id		= !empty( $_POST['id'] ) ? intval( $_POST['id'] ) : 0;
		...
		if( in_array( $current_user->ID, $vote_users) ){
			...
			$count--;
			update_post_meta($id, $db_key, $count);
			$json['message'] = esc_html__('Your vote has removed', 'cannalisting_core');
		} else{
			do_action('fw_add_user_to_votes',$id);
			$count++;
			update_post_meta($id, $db_key, $count);
			$json['message'] = esc_html__('Your vote has update', 'cannalisting_core');
		}
    ...
    add_action('wp_ajax_fw_ext_cannalisting_update_likes', 'fw_ext_cannalisting_update_likes');
    add_action('wp_ajax_nopriv_fw_ext_cannalisting_update_likes', 'fw_ext_cannalisting_update_likes');
}
 ...
 * @return {}
 */
if (!function_exists('cannalisting_set_question_views')) {

    function cannalisting_set_question_views($post_id = '', $key = '') {
    ...
    add_action('sp_set_question_views', 'cannalisting_set_question_views', 2, 10);
}
 ...
 * @load more questions
 */
if (!function_exists('fw_ext_cannalisting_loadmore_questions')) {

    function fw_ext_cannalisting_loadmore_questions() {
			...
			while ($q_query->have_posts()) : $q_query->the_post();
				...
				<div class="tg-question">
					<div class="tg-questioncontent">
						<div class="tg-answerholder spq-v2">
							<?php
								if (isset($category_icon['type']) && $category_icon['type'] === 'icon-font') {
									...
									if (!empty($category_icon['url'])) {
										?>
										<figure class="tg-docimg"><em><img src="<?php echo esc_url($category_icon['url']); ?>" alt="<?php esc_html_e('category', 'cannalisting_core'); ?>"></em></figure>
										<?php
									}
								}
							?>
							...
							<div class="tg-description">
								<p><?php cannalisting_prepare_excerpt('255','false');?></p>
							</div>
							<div class="tg-questionbottom">
								<a class="tg-btn" href="<?php echo esc_url(get_permalink()); ?>">  <?php esc_html_e('Add/View Answers', 'cannalisting_core'); ?> </a>
					...
					<div class="tg-matadatahelpfull">
						...
						<?php fw_ext_get_votes_html($post->ID,esc_html__('Is this helpful?', 'cannalisting_core'));?>
					</div>
				</div>
			<?php
			endwhile;
    ...
    add_action('wp_ajax_fw_ext_cannalisting_loadmore_questions', 'fw_ext_cannalisting_loadmore_questions');
    add_action('wp_ajax_nopriv_fw_ext_cannalisting_loadmore_questions', 'fw_ext_cannalisting_loadmore_questions');
}

/**
 * @load more questions
 */
if (!function_exists('fw_ext_cannalisting_loadmore_answers')) {

    function fw_ext_cannalisting_loadmore_answers() {
        global $current_user, $wp_roles, $userdata;
        $json = array();
		$page_no	= !empty( $_POST['page'] ) ? esc_attr( $_POST['page'] ) : '';
		...
		if ( $a_query->have_posts() ) :
			while ($a_query->have_posts()) : $a_query->the_post();
				...
				$user_avatar = apply_filters(
						'cannalisting_get_media_filter', 
						cannalisting_get_user_avatar(array('width' => 100, 'height' => 100), $answer_user_id), 
						array('width' => 100, 'height' => 100) //size width,height
					);

				$user_name  = cannalisting_get_username($answer_user_id);
				$author_url	= get_author_posts_url($answer_user_id);
				?>
				<div class="tg-answerholder">
					<figure class="tg-docimg">
						<?php if (apply_filters('cannalisting_do_check_user_type', $answer_user_id) === true){?>
					...
					<div class="tg-question">
							...
							<div class="tg-questionbottom">
								<?php if (apply_filters('cannalisting_do_check_user_type', $answer_user_id) === true){?>
									<?php if( intval( $answer_user_id ) !== intval( $current_user->ID ) ){?>
										<a target="_blank" class="tg-btn" href="<?php echo esc_url($author_url); ?>"><?php esc_html_e('Consult Now', 'cannalisting_core'); ?> </a>
									<?php }?>
								<?php }?>
						...
						<div class="tg-matadatahelpfull">
							...
							<?php fw_ext_get_votes_html($post->ID,esc_html__('Was this answers helpful?', 'cannalisting_core'));?>
						</div>
					</div>
				</div>
			<?php
    ...
    add_action('wp_ajax_fw_ext_cannalisting_loadmore_answers', 'fw_ext_cannalisting_loadmore_answers');
    add_action('wp_ajax_nopriv_fw_ext_cannalisting_loadmore_answers', 'fw_ext_cannalisting_loadmore_answers');
}
/**
 * 
 */
//if (!function_exists('fw_ext_register_cannalisting_questions_menu')) {
//
//    function fw_ext_register_cannalisting_questions_menu() {
//        add_menu_page(esc_html__('cannalisting Questions & Answers', 'cannalisting_core'), esc_html__('cannalisting Q&A', 'cannalisting_core'), 'manage_options', 'cannalisting-quesandans', 'cannalisting_render_menu_page', 'dashicons-format-status', 15);
//    }
//
//    function cannalisting_render_menu_page() {
...
//    }
//
//    add_action('admin_menu', 'fw_ext_register_cannalisting_questions_menu');
//}